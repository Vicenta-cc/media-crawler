...
class KuaishouCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if (
                ...
                utils.logger.info(
                    "[KuaishouCrawler] Saved login state is unavailable, "
    ...
    def _resolve_background_headless(self) -> bool:
        ...
        utils.logger.info(
            "[KuaishouCrawler] Background browser mode: "
    ...
    async def _launch_browser_context(
        ...
        if use_background_mode:
            utils.logger.info(
                "[KuaishouCrawler] Launching browser using background mode "
    ...
    async def search(self):
        ...
        if not config.STREAM_ITEMS and config.CRAWLER_MAX_NOTES_COUNT < ks_limit_count:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while crawled_count < config.CRAWLER_MAX_NOTES_COUNT:
                ...
                for video_detail in feeds[:remaining]:
    ...
    async def fetch_creator_video_detail(self, video_list: List[Dict]):
        ...
        if config.STREAM_ITEMS:
            semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
            task_list = [
                self.process_creator_video_stream_item(post_item, semaphore)
    ...
    async def process_creator_video_stream_item(self, post_item: Dict, semaphore: asyncio.Semaphore) -> None:
        """Process one creator video fully, then persist it for streaming consumers."""
        video_id = post_item.get("photo", {}).get("id")
        if not video_id:
            return

        video_detail = await self.get_video_info_task(video_id, semaphore)
        if video_detail is None:
            video_detail = post_item
