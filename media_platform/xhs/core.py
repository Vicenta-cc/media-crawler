...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            if (
                ...
                utils.logger.info(
                    "[XiaoHongShuCrawler] Saved login state is unavailable, "
    ...
    def _has_saved_login_state(self) -> bool:
        cookie_domain = "rednote.com" if config.XHS_INTERNATIONAL else "xiaohongshu.com"
    ...
    async def _launch_browser_context(
        ...
        if use_background_mode:
            utils.logger.info(
                "[XiaoHongShuCrawler] Launching browser using background mode "
    ...
    async def search(self) -> None:
        ...
        if not config.STREAM_ITEMS and config.CRAWLER_MAX_NOTES_COUNT < xhs_limit_count:
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while crawled_count < config.CRAWLER_MAX_NOTES_COUNT:
                ...
                try:
                    ...
                    search_items = search_items[:remaining]
    ...
    async def process_search_note_stream_item(self, post_item: Dict) -> None:
        note_id = post_item.get("id")
        xsec_source = post_item.get("xsec_source")
        xsec_token = post_item.get("xsec_token")
        if not note_id:
        ...
        await self.get_notice_media(note_detail)
        await self.batch_get_note_comments([note_detail.get("note_id")], [note_detail.get("xsec_token")])
        await xhs_store.update_xhs_note(note_detail)
    ...
    async def fetch_creator_notes_detail(self, note_list: List[Dict]):
        ...
        if config.STREAM_ITEMS:
            semaphore = asyncio.Semaphore(config.MAX_CONCURRENCY_NUM)
            task_list = [
                self.process_creator_note_stream_item(post_item, semaphore)
    ...
    async def process_creator_note_stream_item(self, post_item: Dict, semaphore: asyncio.Semaphore) -> None:
        """Process one creator note fully, then persist it for streaming consumers."""
        note_id = post_item.get("note_id")
        xsec_source = post_item.get("xsec_source")
        xsec_token = post_item.get("xsec_token")
        if not note_id:
        ...
        await self.get_notice_media(note_detail)
        await self.batch_get_note_comments([note_id], [xsec_token])
        await xhs_store.update_xhs_note(note_detail)
