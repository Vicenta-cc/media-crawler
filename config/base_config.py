...
# 后台浏览器模式：保存过登录态时自动隐藏浏览器运行；需要扫码/人工验证时再启动可见浏览器。
# 开启后，XHS 会优先使用 MediaCrawler 的本地持久化浏览器 profile，避免每次等待 CDP 连接已有浏览器。
ENABLE_BACKGROUND_BROWSER_MODE = True
...
# Whether to process and persist crawled items one by one instead of page/batch mode.
# This is useful for downstream consumers that watch JSONL output and analyze items as they arrive.
STREAM_ITEMS = False
